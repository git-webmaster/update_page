...
function init() {
	...
	let token = window.page.suggestions_token;
...
$(document).ready(function () {
	...
	// set prices
	prices.html(window.page.prices.add);
	...
	submit_button.click(async function (e) {
		...
		var success_template = '<div class="syst">' +
			...
			'\t\t\t\t</figure>' +
			'\t\t\t\t<span class="syst__error-title">Заявка на добавление успешно зарегистрирована, спасибо!</span>' +
			'\t\t\t\t<p class="syst__error-caption">Номер вашей заявки: <span id="issue">%issue%</span>' +
			'<br/><br/>Если всё хорошо — через некоторое время страница станет доступна по этой ссылке: <br/><a href="/r/%link%" target="_blank">https://rubrikator.org/r/%link%</a></p>' +
		...
		$.ajax({
			type: "POST",
			cache: false,
			url: 'https://rubrikator.local/api/ajax/new_request',
			data: {
				token: 'tk',
				owner_id: window.page.id,
				data: data
